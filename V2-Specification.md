...
| [senderAddress]()     | address | Address that is allowed to call Exchange contract methods that take orders as inputs. If set to 0, any address is allowed to call these methods. |
...
| [makerAssetData]()    | bytes   | Encoded data that can be decoded by a specified proxy contract when transferring makerAsset. The last byte must reference the id of this proxy.  |
| [takerAssetData]()    | bytes   | Encoded data that can be decoded by a specified proxy contract when transferring takerAsset. The last byte must reference the id of this proxy.  |
...
TODO: What is the difference between Illegal and Invalid?
TODO: Note on signature lengths
